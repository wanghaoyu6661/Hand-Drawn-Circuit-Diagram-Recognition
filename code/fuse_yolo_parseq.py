...
"""
fuse_yolo_parseq.py
整合 YOLO 检测结果与 PARSeq OCR 文本识别

【最终版本（治本）】
- 只做 OCR + “类别级别”的近邻提示（matched_component_cls_id）
- 不再输出实例级 matched_component_id（因为此阶段根本没有稳定的 component 实例 id）
- 实例级绑定放到 build_final_json.py：用最终 components 的 bbox 去匹配 texts

输出 fuse_json: list[dict]
{
  "image": image_name,
  "text": pred_text,
  "text_box": [xc, yc, bw, bh],        # YOLO normalized xywh
  "match_relation": "component" | "global_description",
  "distance": float(min_dist),         # normalized distance in YOLO space
  "matched_component_cls_id": int | None,
  "component_cls_name": str | None,
  "matched_component_id": None         # 保留字段但恒为 None，避免下游误用
}
"""
...
import sys
sys.path.append("/root/autodl-tmp/parseq-main")
...
def load_parseq_model(ckpt_path, charset_path, device):
    ...
    model = torch.hub.load(
        '/root/.cache/torch/hub/baudm_parseq_main',
        'parseq',
        source='local',
        pretrained=False
    )
...
def main():
    ...
    output_dir = "/root/autodl-tmp/final_result/fuse_json"
    ckpt = "/root/autodl-tmp/handocr_project/output_parseq/best_parseq.pt"
    charset_path = "/root/autodl-tmp/handwritten_ocr/scripts/my_dict.txt"
    data_yaml = "/root/yolov9-main/data/data.yaml"
