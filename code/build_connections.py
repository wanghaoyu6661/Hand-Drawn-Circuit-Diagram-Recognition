# -*- coding: utf-8 -*-
"""
build_connections.py  (NEW: nearest-first + pixel connectivity + angle-gap constraint)

核心变化（相对你上一版）：
- ✅ 不再用 SECTOR_COUNT/扇区桶（避免“卡边界”问题）
- ✅ 对每个 node 维护“已连接边的真实角度列表”
- ✅ 新候选边若与任一已连边夹角 < MIN_ANGLE_SEP_DEG，则拒绝（角度间隔约束）
- ✅ 仍然：距离从近到远尝试 + 像素连通率 ratio 判定 + terminal 一端一线 + node 最多4条

输出 JSON 结构保持不变：build_final_json.py 可直接消费
"""
...
# ========= 基本路径 =========
MERGED_JSON_DIR = "/root/autodl-tmp/final_result/merged_points/merged_points_json"
LAMA_IMG_DIR    = "/root/autodl-tmp/final_result/lama_clean"
DAOXIAN_DIR     = "/root/autodl-tmp/final_result/dao_xian"

OUT_BASE   = "/root/autodl-tmp/final_result/link"
...
CONNECT_RATIO_T = 0.7       # 连通阈值（你想用0.95就改这里）
